...
Данное тестовое задание я уже отправлял на поток стажировки летом, в связи с чем я решил внести необходимые по личному мнению изменения и усовершенствования в программу для демонстрации моего прогресса.

Список внесенных изменений:

- Проверенная поддержка кросс-платформенности и использование pathlib для работы с директориями
- Созданы dockerfile и dockerimage, который опубликован в разделе package GitHub, с помощью него программа установлена на собственный сервер
- Реализовано автотестирование с помощью pytest разных уровней для программы (unit-тестирование, интеграционное тестирование, end-to-end тестирование) для демонстрации навыков и знаний в области тестирования
- Исходя из результатов тестирования, внесены различные изменения в программу чтобы поведение API совпадало с желаемым
- Реализована экспериментальная поддержка многопоточности (файлы доступны на GitHub в отдельной ветке multithreading), возможны критические баги, не использовать для проверки

## Проверка работы приложения 

Кроме локального запуска сервера вы можете использовать следующие ссылки для доступа к API http://83.222.9.91:8000/ и http://rkedjk.duckdns.org:8000/

Интерактивная документация доступна по следующим ссылкам http://83.222.9.91:8000/docs и http://rkedjk.duckdns.org:8000/docs

## Начало работы

Для запуска сервера существуют две опции:
- Запуск напрямую из исходного кода  
- Запуск через образ Docker

Ниже описаны инструкции для обоих случаев 
## Запуск приложения из исходного кода 

Чтобы начать использование приложения, выполните следующие шаги:

1. Склонируйте репозиторий на свой компьютер или загрузите архив.

2. Убедитесь, что у вас установлен Python версии 3.11.6. Рекомендуется создать виртуальное окружение (venv) и выполнять все команды из него. Если у вас нет venv, создайте его:
    ...
    - На Windows:
      ...
      При возникновении проблем на этом этапе, обратиться к [документации](https://docs.python.org/3/library/venv.html)

3. Установите необходимые зависимости, перечисленные в файле `requirements.txt`, с помощью pip:
...
4. Запустите приложение с использованием предоставленного скрипта:
    ...
    - На Windows, запустите `start_windows.bat`, дважды щелкнув по нему (если venv не использовался) или запустив из командной строки:

      ```batch
      .\start_windows.bat
      ```

    Если после запуска сервер не включен, запустите его вручную (рекомендуемый способ):

    ```bash
    uvicorn main:app --reload
    ```
    Данный метод запускает программу в режиме разработчика, что позволяет изменять и перезапускать ее на ходу 

    Если же нужно запустить программу в стандартном режиме воспользуйтесь следующей командой (не рекомендуемый способ):  
    ```bash
...
Для использования Docker выполните следующие шаги:

1. Загрузите Docker-образ из репозитория пакетов GitHub:

    ```bash
    docker pull ghcr.io/rkedjk/python-test-task:main
    ```

2. Запустите контейнер:

    ```bash
    docker run -d --name python-task -p 8000:8000 ghcr.io/rkedjk/python-test-task:main
    ```

## Проверка работы
После этого сервер будет доступен по адресу [127.0.0.1:8000](http://127.0.0.1:8000/), а интерактивная документация по адресу [http://127.0.0.1:8000/docs](http://127.0.0.1:8000/docs).
## Особенность 
...
Pytest - библиотека автотестирования
...
Если тестовое задание не пройдет проверку. Очень прошу отправить фидбек в целях повышения навыков на почту - rkedjk@yandex.ru
